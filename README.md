...
| 完成 | 周 | 阶段 | 本周目标 |
| --- | --- | --- | --- |
| - [x] | [W1](#week-1) | A | 第一次 DeepSeek 调用，测试点 JSON 过 Pydantic |
...
- [x] **本周验收** · 能独立用 PRD 得到通过 schema 的 JSON；缺字段会被拦住
...
- [x] D2 · Few-shot 正例 + 反例；PRD → 用例
- [x] D3 · Bug 分析必须绑证据；无日志不得给根因
- [x] D4 · Prompt 文件化；`prompts/render.py`
- [x] D5 · 黄金集起步 5 条 jsonl
- [x] D6 · 人工三项打分（可执行性 / 覆盖 / 幻觉）
- [x] D7 · 复盘并升 v1.1，保留 v1
